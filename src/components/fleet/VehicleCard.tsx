...
export function VehicleCard({ vehicle, computedStatus, activeSeasons, allRules }: VehicleCardProps) {
  ...
  return (
    <Link to={`/vehicle/${vehicle.id}`} className="block bg-white rounded-lg shadow-card overflow-hidden transition-all hover:shadow-md active:shadow-sm focus:outline-none focus:ring-2 focus:ring-flitx-blue-200">
      <div className="relative">
        {vehicle.image ? (
          <img src={vehicle.image} alt={`${vehicle.year} ${vehicle.make} ${vehicle.model}`} className={cn("h-40 w-full object-scale-down", vehicle.is_sold && "opacity-60")} />
        ) : (
          <div className={cn("h-40 w-full bg-flitx-gray-100 flex items-center justify-center", vehicle.is_sold && "opacity-60")}>
      ...
      <div className="p-4">
        ...
        {vehicle.is_sold && vehicle.sale_price != null ? (
          <div className="mt-3 text-sm font-medium text-red-600">
            {t('soldFor')}: €{Number(vehicle.sale_price).toLocaleString()}
          </div>
        ) : (
