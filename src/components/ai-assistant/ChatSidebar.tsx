...
export function ChatSidebar({
  ...
  return <div className="w-64 h-full bg-white/80 backdrop-blur-xl border-r border-blue-100/50 flex flex-col">
      {/* New Chat Button */}
      <div className="p-4">
      ...
      <div className="flex-1 overflow-y-auto px-2">
        ...
        <div className="space-y-1">
          {conversations.map(conv => <div key={conv.id} className={cn("group relative flex items-center gap-2 px-3 py-2.5", "rounded-lg cursor-pointer", "transition-all duration-150", activeConversation === conv.id ? "bg-blue-50 text-blue-900" : "hover:bg-gray-50 text-gray-700")} onClick={() => onSelectConversation(conv.id)}>
          ...
          }} className={cn("absolute right-2 opacity-0 group-hover:opacity-100", "p-1.5 rounded-md", "hover:bg-red-50 text-gray-400 hover:text-red-500", "transition-all duration-150")}>
                <Trash2 className="w-3.5 h-3.5" />
              </button>
            </div>)}

          {conversations.length === 0 && <p className="text-sm text-gray-400 px-3 py-4 text-center">
      ...
      <div className="p-4 border-t border-blue-100/50 rounded-2xl">
        ...
        <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-blue-500 to-blue-600 rounded-full transition-all duration-300" style={{
